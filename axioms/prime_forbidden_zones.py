...
fracs_1 = [sp.totient(p-1) / (p-1) for p in primes_1]
plt.figure(1)
...
fracs_2 = np.array([sp.totient(p-1)/(p-1) for p in primes_2])
fermat_fracs = [sp.totient(p-1)/(p-1) for p in fermat_primes if p in primes_2]
plt.figure(2)
...
fracs_3 = np.array([sp.totient(p-1)/(p-1) for p in primes_3])
...
fracs_4 = np.array([sp.totient(p-1)/(p-1) for p in primes_4])
...
candidates = [n for n in range(100, 1000) if sp.totient(n-1)/(n-1) > 0.45]
...
phis = [sp.totient(p-1) for p in primes_6]
...
betas = np.array([sp.totient(p-1)/(p-1) for p in primes_7])
...
fracs_8 = np.array([sp.totient(p-1)/(p-1) % 1 for p in primes_8])  # Fractional mod 1
...
x = np.log(primes_9)
y = [sp.totient(p-1)/(p-1) for p in primes_9]
dx = np.gradient(x)
dy = np.gradient(y)
plt.figure(9)
...
fracs_10 = np.array([sp.totient(p-1)/(p-1) for p in primes_10])
